...
module.exports.run = async (client, message, args) => {
  ...
  if(!sebep) return message.channel.send(new MessageEmbed().setAuthor(message.author.username, message.author.displayAvatarURL).setColor(message.member.displayHexColor).setDescription("Ceza sebebini girmek zorundasınız.").setFooter(`Lore ❤️`)).then(e => e.delete({timeout:10000}));; db.delete("jail_" + member.id) 
  ...
  if(kontrol) return message.channel.send(new MessageEmbed().setAuthor(message.author.username, message.author.avatarURL()).setColor(message.member.displayHexColor).setDescription("Kullanıcı zaten cezalı.").setFooter(`Lore ❤️`)).then(e => e.delete({timeout:10000}));;

  message.channel.send(new MessageEmbed().setAuthor(message.author.username, message.author.avatarURL({ dynamic: true})).setColor(message.member.displayHexColor).setDescription("<@" + member + "> (`" + member.id +"`) üyesine <@&"+ayarlar.Cezalı+"> rolü verildi.")).then(e => e.delete({timeout:10000}));;
  ...
  member.roles.add(ayarlar.Karantina);

  client.channels.cache.get(ayarlar.CezalıLog).send(new MessageEmbed().setAuthor(message.guild.name, message.guild.iconURL({dynamic: true})).setFooter(ayarlar.prefix+`cezasorgu ID komudunu kullanarak, kişinin cezalarını sorgulayabilirsiniz.`).setThumbnail(member.user.avatarURL({dynamic: true})).setColor(message.member.displayHexColor).setDescription(`${member} üyesine <@&`+ayarlar.Karantina+`> rolü verildi. \n\n• Yetkili: <@`+ message.author.id +`> (\``+message.author.id+`\`)\n• Tarih: \`${cezaatılma}\` \n• Sebep: \`${sebep}\``).setFooter(`Lore ❤️`))
