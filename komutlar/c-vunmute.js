...
module.exports.run = async(client, message, args) => {
    ...
    if(!mention.voice.setMute(true)) return message.channel.send(new MessageEmbed().setDescription(`${mention} (\`${mention.id}\`) adlı kullanıcı susturulmuş değil.`).setFooter(`Lore ❤️`))
    if(mention.voice.channel) { 
     db.delete(`seslide2.${mention.user.id}.${message.guild.id}`)
     mention.voice.setMute(false);
     message.channel.send(new MessageEmbed().setDescription(`${mention} (\`${mention.id}\`) adlı kullanıcının ses kanallarındaki susturulması <@`+message.author.id+`> yetkilisi tarafından kaldırıldı`).setFooter(`Lore ❤️`))
    }
