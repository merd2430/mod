...
if(!message.member.roles.cache.has(ayarlar.VMuteYetki) && !message.member.hasPermission("ADMINISTRATOR")) return;
...
if(mention.voice.channel) {
  ...
  message.channel.send(ayarlar.MuteEmoji +`${mention} ${time} boyunca ses kanallarında susturuldu. (\`#${cezasayısı}\`)`)

  mention.voice.setMute(true)

  client.channels.cache.get(ayarlar.VMuteLog).send(new MessageEmbed()
  .setColor('GREEN')
  .setAuthor(message.author.tag, message.author.avatarURL ({ dynamic: true }))
  .setDescription(ayarlar.MuteEmoji+`${mention} (\`${mention.user.id}\`) üyesi ses kanallarında susturuldu.
...
`).setFooter(`Lore ❤️`))
  ...
  setTimeout(async () => {
    ...
    client.channels.cache.get(ayarlar.VMuteLog).send(
    new MessageEmbed()
    .setColor('GREEN')
    .setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true }))
  ...
  .setColor('RED')
  .setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true }))
  .setDescription(ayarlar.unmuteemoji +`${mention} (\`${mention.user.id}\`) üyesinin ses kanallarında bulunan susturulması kaldırıldı.
