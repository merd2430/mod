    ...
    if (!muted) {
        ...
        if (muted.roles.highest.position >= message.member.roles.highest.position) {
            ...
            if (!mutezaman) {
                ...
                try {
                    client.channels.cache.get(ayarlar.MuteLog).send(new MessageEmbed().setAuthor(message.author.tag, message.author.avatarURL({dynamic: true})).setColor(`GREEN`).setDescription( ayarlar.MuteEmoji +` <@${kişi.id}> (\`${kişi.id}\`) üye metin kanallarında susturuldu.
...
• Sebep: \`${sebep}\``).setFooter(`Lore ❤️`));
                    muted.roles.add(ayarlar.muteli);
...
db.push(`ceza.${member.id}`, {
    ...
    type: cezatipi
                ...
                setTimeout(async function () {
                    muted.roles.remove(
                        ayarlar.muteli,
                        client.channels.cache.get(ayarlar.MuteLog).send(
                            new MessageEmbed()
                            .setColor('GREEN')
                            .setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true}))
                            .setDescription(ayarlar.UnMuteEmoji +` <@${kişi.id}>(\`${kişi.id}\`) üyesinin metin kanallarında susturulma cezası sonlandı.
